...
interface ClientPageProps {
    initialDotzeeState: Record<string, any>;
}

let isHydrated = false; // Prevent re-hydration on HMR

export default function ClientPage({ initialDotzeeState }: ClientPageProps) {
    ...
    useEffect(() => {
        if (!isHydrated) {
            ...
            if (process.env.NODE_ENV === 'development') {
                enableDotzeeDevTools({
                    name: 'Dotzee Next.js Example',
                    trace: true,
                });
            }
            isHydrated = true; // Mark as hydrated
            console.log('[Dotzee Hydration] Client hydrated.');
        }
    }, [initialDotzeeState]);
